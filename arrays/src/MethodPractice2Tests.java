...
public class MethodPractice2Tests {
    ...
    void returnNegativeNumbers() {
        ...
        assertArrayEquals(negativeNumbers, MethodPractice2.filterNegativeValues(11, 56, 0, -99, -23, 2, -78, -1));
    }

    @Test
    void returnSumOfCountNegativeElements() {
        assertEquals(2, MethodPractice2.countNegativeElements(1, -2, 3, -4));
    }
    ...
    @ParameterizedTest
    @CsvSource({"3,9", "0,0", "16,32", "18,54", "1,3"})
    void returnModifiedValue(int number, int expected) {
        assertEquals(expected, MethodPractice2.calculateModifiedValue(number));
    }

    @ParameterizedTest
    @ValueSource(ints = {0, 1, 38, -5, -89})
    void isPrimeNumberFalse(int number) {
        assertFalse(MethodPractice2.isPrimeNumber(number));
    }

    @ParameterizedTest
    @ValueSource(ints = {2, 3, 5, 89})
    void isPrimeNumberTrue(int number) {
        assertTrue(MethodPractice2.isPrimeNumber(number));
    }
    ...
    void returnWeirdPercent(int number, double expected) {
        assertEquals(expected, MethodPractice2.calculateWeirdPercent(number));
    }

    @ParameterizedTest
    @CsvSource({"0,0", "-1,-0.3", "100,30", "300,90", "850_000,255_000"})
    void returnThirtyPercent(int number, double expected) {
        assertEquals(expected, MethodPractice2.calculateThirtyPercent(number));
    }
}